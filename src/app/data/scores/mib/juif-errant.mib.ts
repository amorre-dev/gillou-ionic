...
const JuifErrantMib = (): SheetData => {
  // Measure 1
  const note1_m1 = new StaveNote({ keys: ['b/4'], duration: 'qr' });
  const note2_m1 = new StaveNote({ keys: ['c/4'], duration: 'q' });

  // Measure 2
  const note1_m2 = new StaveNote({ keys: ['a/4'], duration: '8' });
  const note2_m2 = new StaveNote({ keys: ['g/4'], duration: '8' });
  const note3_m2 = new StaveNote({ keys: ['f/4'], duration: '8' });
  const note4_m2 = new StaveNote({ keys: ['d/4'], duration: '8' });
  const beam1_m2 = new Beam([note1_m2, note2_m2, note3_m2, note4_m2]);

  // Measure 3
  const note1_m3 = new StaveNote({ keys: ['c/4'], duration: '8' });
