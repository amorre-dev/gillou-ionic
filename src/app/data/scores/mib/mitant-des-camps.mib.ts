...
const MitantDesCampsMib = (): SheetData => {
  ...
  const note3_m2 = dotted(new StaveNote({ keys: ['a/4'], duration: '8d' }));
  const note4_m2 = new StaveNote({ keys: ['g/4'], duration: '16' }).addModifier(
    new Accidental('#')
  );

  const note1_m3 = new StaveNote({ keys: ['a/4'], duration: '2' });
